...
$work{cluster}->{connection}{0}->{ip} = "172.16.0.1";
$work{cluster}->{connection}{1}->{ip} = "172.16.0.2";
$work{cluster}->{OID}                 = '.1.3.6.1.4.1.5951.4.1.3.1.1.59';             # Список Virtual Servers
$work{cluster}->{OID_EXCLUDE}         = 'snmpv2-smi::enterprises.5951.4.1.3.1.1.59.'; # Исключение OID
$work{cluster}->{TMP_FILE_DROP}       = 1;

# Шаблон который будет использоватся, при создании и учете хостов в мониторинге
$work{template}->{name}  = 'Template_NetScaler_VPX_LB_vServer';
...
$work{create}->{hostgroup}->{name}      = 'NetScaler VPX LB vServer';  # Хост группа, в которую будут добавлятся хосты
$work{create}->{hostgroup}->{id}        = undef;

$work{create}->{z_proxy}->{name}        = 'Inet';  # Имя прокси сервера
...
foreach my $id ( keys %{ $work{cluster}->{connection} } ) {
  if ( ! defined $work{cluster}->{connection} ) {
...
foreach my $vserver ( keys %{ $work{vserver} } ) {
  if ( defined $work{vserver}{$vserver}->{result} and $work{vserver}{$vserver}->{result} eq "hostcreate" and defined $work{vserver}{$vserver}->{zabbix_host} ) {
    ...
    print "New host ID $hostids $zabbix_host  - $zabbix_name\n";
    }
  select(undef,undef,undef, 0.20); # пауза обязательна, для исключения задвоение ID ролей LLD и прототипов
  }
...
# Следующий этап изменения метрик
...
# Проводим анализ, правильности подключения метрик к интерфейсам
foreach my $hostid ( keys %{ $work{zabbix}{template}{host} } ) {
